...
class Cron {
	...
	scheduleOneMinute() {
		cron.schedule("* * * * *", async () => {
			await database();
			const monitors = await Monitor.find({ heartbeat: 1, enabled: true });
	...
	scheduleFiveMinutes() {
		cron.schedule("*/5 * * * *", async () => {
			await database();
			const monitors = await Monitor.find({ heartbeat: 5, enabled: true });
			this.executeJobs(monitors);
		});
	}
	...
	scheduleTenMinutes() {
		cron.schedule("*/10 * * * *", async () => {
			await database();
			const monitors = await Monitor.find({ heartbeat: 10, enabled: true });
			this.executeJobs(monitors);
		});
	}
	...
	scheduleThirtyMinutes() {
		cron.schedule("*/30 * * * *", async () => {
			await database();
			const monitors = await Monitor.find({ heartbeat: 30, enabled: true });
			this.executeJobs(monitors);
		});
	}
	...
	scheduleHour() {
		cron.schedule("0 * * * *", async () => {
			await database();
			const monitors = await Monitor.find({ heartbeat: 60, enabled: true });
			this.executeJobs(monitors);
		});
	}
